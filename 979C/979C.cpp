...
#endif
/**************CODE_BEGINS_HERE************************************************/
vi inTime, outTime, subtreeSize, parent;
int st = 0;

void dfs(int curr, int p, const vector<vector<int>>& adj) 
{
    parent[curr] = p;
    subtreeSize[curr] = 1;
    st++;
    ...
    for (auto neighbour : adj[curr]) 
    {
        if (neighbour != p) 
        {
            dfs(neighbour, curr, adj);
            subtreeSize[curr] += subtreeSize[neighbour];
        }
    }
...
void solve() {
    ...
    if (inTime[x] <= inTime[y] && outTime[x] >= outTime[y]) 
    {
        isChild = true;
        int child_on_path = y;
        while (parent[child_on_path] != x)     //consider if x is direct parent of y, then it will never enter in this loop and a= n - subtreeSize[y] which is correct
        {
            child_on_path = parent[child_on_path];
        }
        int a = n - subtreeSize[child_on_path];
        int b = subtreeSize[y];
        total -= a * b;
    }
